...
| Acquisition | **YouTube subs first** (in the video's detected audio language; English subs fall back only if none in the original language) **→ Whisper fallback** only when no subs exist (Whisper also runs in the detected language, never `auto`) | Always-Whisper (rejected after dogfooding — see "Dogfood findings" below); always-YouTube-subs (rejected — some videos genuinely have no subs); English-first priority list regardless of video language (rejected — produces poorly-segmented or empty transcripts on Mandarin/Japanese/Korean content) |
| Language detection | From `info.json`: first non-null of `language` → first key of `subtitles` → first key of `automatic_captions` → CJK-ratio heuristic on title+description | Always-English (loses any non-English content); audio-snippet Whisper detection (requires downloading audio just to pick a sub-lang priority — wasteful) |
