...
description: Use when optimizing PyTorch training speed or memory on CUDA GPUs — global flags, torch.compile, fused optimizers, mixed precision, gradient checkpointing, kernel fusion, memory layout, latent-space training, or FID evaluation during training. Applies to any PyTorch training workload.
---
...
| FID: 5K samples (training) | 10x faster than 50K | Less | `sample_fid_n: 5000` |
| FID: dopri5 adaptive ODE | Faster than fixed-step | Same | `sampling_method: dopri5` |
| FID: latent-space generation | 64x less compute | Much less | `is_latent: true` |
| FID: multi-metric single pass | Amortizes sampling | Same | Compute FID+IS+KID together |
